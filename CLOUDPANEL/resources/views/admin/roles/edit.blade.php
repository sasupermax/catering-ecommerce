    ...
    <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="bg-white rounded-lg shadow p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="slug" value="{{ old('slug', $role->slug) }}" required
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                {{ in_array($role->slug, ['admin', 'editor', 'viewer']) ? 'readonly' : '' }}>
            @if(in_array($role->slug, ['admin', 'editor', 'viewer']))
            <p class="text-xs text-amber-600 mt-1">⚠️ No puedes modificar el slug de roles del sistema</p>
            @else
            <p class="text-xs text-gray-500 mt-1">Usar solo minúsculas, números y guiones</p>
            @endif
            @error('slug')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">{{ old('description', $role->description) }}</textarea>
            @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
